...
from ROOT import RooWorkspace, TFile, TH1, TChain, RooDataHist, \
    ...
    RooArgSet, RooArgList, TH2F, TH2D, TTree, TF2, TH1F, TH1D, TF1, TH3F, TH3D, TF3, RooFormulaVar, TCanvas, TProfile2D, kRed
...
#    ws = RooWorkspace(workspaceName)    
    ...
    #additional parameters in tree that have to be zero
    cut_string = ''
    for more_para in cfg.get('Global','morepars').split(','):
	if (more_para == ''):
		cut_string = ''
	else:
		cut_string += ' && ' + more_para + ' == 0 '
	
    #non-uniform binning?only for 1d atm
    non_uni_bins	= int(cfg.get('Global','nonuniBins'))
    ...
    #bins given in list to allow non-uniform binning
    if (model=="par1_TH1" or model=="par1_TF1"):
        ...
        par1Bins	= []
        for i in range(nGridPar1Bins+1):
	    par1Bins.append(par1GridMin + i*(par1GridMax-par1GridMin)/nGridPar1Bins)
	par1Bins	= array('f',par1Bins)
	if non_uni_bins:
	    par1Bins	= array('f',[float(i) for i in cfg.get('Global','par1Bins').split(',')])
	...
	for i in range(nGridPar1Bins+1):
	    par1Bins.append(par1GridMin + i*(par1GridMax-par1GridMin)/nGridPar1Bins)
	par1Bins	= array('f',par1Bins)
	for i in range(nGridPar2Bins+1):
	    par2Bins.append(par2GridMin + i*(par2GridMax-par2GridMin)/nGridPar2Bins)
	par2Bins	= array('f',par2Bins)
	if non_uni_bins:
	    par1Bins	= array('f',[float(i) for i in cfg.get('Global','par1Bins').split(',')])
	    par2Bins	= array('f',[float(i) for i in cfg.get('Global','par2Bins').split(',')])
	...
	for i in range(nGridPar2Bins+1):
	    par2Bins.append(par2GridMin + i*(par2GridMax-par2GridMin)/nGridPar2Bins)
	par2Bins	= array('f',par2Bins)
	for i in range(nGridPar3Bins+1):
	    par3Bins.append(par3GridMin + i*(par3GridMax-par3GridMin)/nGridPar3Bins)
	par3Bins	= array('f',par3Bins)
	if non_uni_bins:
	    par1Bins	= array('f',[float(i) for i in cfg.get('Global','par1Bins').split(',')])
	    par2Bins	= array('f',[float(i) for i in cfg.get('Global','par2Bins').split(',')])
	    par3Bins	= array('f',[float(i) for i in cfg.get('Global','par3Bins').split(',')])
...
#    obs_mc = ws.var('%s_%s'%(cfg.get('Global','obsVar'),section))    
	...
	if section[-1] != "8":#i != len(bins) - 1:
		...
		sigObj.Draw(par2Name+'*1e12'+':'+par1Name+'*1e12'+' >> theBaseData_'+section+'_'+str(i),
			    weight.GetName()+'*('+cfg.get(section,'obsVar') + #
			    ' > ' + str(binMin) + #
			    ' && ' + cfg.get(section,'obsVar') + #
			    ' < ' + str(binMax) + #
	...
	#last bin includes overflow
		...
		sigObj.Draw(par1Name+'*1e12'+' >> theBaseData_'+section+'_'+str(i),
			    weight.GetName()+'*('+cfg.get(section,'obsVar')+#
			    ' > ' + str(bins[len(bins)-2])+ cut_string +')','goff')		
	    elif (model=="par1par2_TH2" or model=="par1par2_TF2"):
		sigObj.Draw(par2Name+'*1e12 :'+par1Name+' *1e12 >> theBaseData_'+section+'_'+str(i),
			    weight.GetName()+'*('+cfg.get(section,'obsVar')+#
			    ' > ' + str(bins[len(bins)-2])+ cut_string +')','goff')
        ...
        func.SetLineColor(2)
        theBaseData.Fit(func,'R','')
        #func.GetXaxis().SetRangeUser(-32e-12,32e-12)
        #theBaseData.SetBins(theBaseData.GetNbinsX(),(theBaseData.GetXaxis().GetBinLowEdge(1)* 1e-12), (theBaseData.GetXaxis().GetBinUpEdge(theBaseData.GetNbinsX())* 1e-12))
