...
{
  if (parname.EqualTo(param1) )  return "f_{s0} / #Lambda^{4} (TeV^{-4})";
  if (parname.EqualTo(param2) ) return "f_{s1} / #Lambda^{4} (TeV^{-4})";
...
{
  if (parname.EqualTo(param1) )  return -50;
  if (parname.EqualTo(param2) ) return -55;
...
{
  if (parname.EqualTo(param1) )  return 50;
  if (parname.EqualTo(param2) ) return 80;
...
{
  if (parname.EqualTo(param1) )  return 1;
  if (parname.EqualTo(param2) ) return 1;
		     ...
		     TLegend *legend)
{
  ...
  finalPlot->Modified();
  finalPlot->Update();
  finalPlot->Print(Form("docuplots/%s.pdf",plotprefix.Data()));
  finalPlot->Print(Form("docuplots/%s.eps",plotprefix.Data()));
  finalPlot->Print(Form("docuplots/%s.png",plotprefix.Data()));
		    ...
		    float par2_bestfit)
{
  ...
  if (contLevel) {
    ...
    for (int i=0; i<contLevel->GetSize(); i++) {
      curv->Draw("SAME C");
      curv->SetLineWidth(3);
      if (!i) legend->AddEntry(curv,"pseudo observed 95% C.L.","L");
      curv=(TGraph *)(contLevel->After(curv));
    }
  }
...
//   SMpoint -> SetMarkerSize(2);
  ...
  std::cout<< "Best fit value: " + param1 + " " <<par1_bestfit<<" ,  Best fit value: " + param2 + " "<<par2_bestfit<<std::endl;
//   bestfit->Draw("SAME P");
// legend->AddEntry(bestfit,"Best fit value","P");
  ...
  //text->AddText(Form("95%% CL Limit on %s and %s",par2latex(par1).Data(),par2latex(par2).Data()));
  text->AddText(0,0.35,Form("35.9 fb^{-1} (%d TeV)",beamcometev));
  text->Draw();
...
void atgcplotLimit_bestfit()
{
  ...
  */
  ...
  TLegend *legend = new TLegend(0.18,0.75,0.92,0.92,"","NDC");
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
  legend->SetBorderSize(0);
  legend->SetHeader("CMS Private work");
